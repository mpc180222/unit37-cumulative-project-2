...
//Route for job applications. Any logged in user can apply for jobs themselves, and admin users can submit applications for users other than themselves.
...
router.post("/:username/jobs/:id", ensureLoggedIn, async function (req, res, next) {
  try {
    ...
    if(res.locals.user.username !== username && res.locals.user.isAdmin !== true)throw new UnauthorizedError("You must be an admin to apply for a job as a different user.")
